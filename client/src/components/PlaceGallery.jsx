...
export default function PlaceGallery({ place }) {
    ...
    if (showAllPhotos) {
        return (
            <div className="absolute inset-0 bg-black/90 text-white">
                <div className="p-12">
                    ...
                    <div className="grid grid-cols-3 gap-8 mt-8 max-w-[1400px] mx-auto">
                        {place?.photos?.length > 0 && place.photos.map((photo, key) => (
                            <Image key={key} src={photo} alt="" classname={'rounded-xl'} />
                        ))}
                    </div>
